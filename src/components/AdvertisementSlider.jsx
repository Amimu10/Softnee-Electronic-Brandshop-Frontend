// import Slider from 'react-slick';
// const AdvertisementSlider = ({ details }) => {

//   const sliderSettings = {
//     dots: true,
//     infinite: true,
//     speed: 500,
//     slidesToShow: 3,
//     slidesToScroll: 1,
//     autoplay: true,
//     autoplaySpeed: 3000,
//     responsive: [
//       {
//         breakpoint: 768,
//         settings: {
//           slidesToShow: 2,
//         },
//       },
//       {
//         breakpoint: 480,
//         settings: {
//           slidesToShow: 1,
//         },
//       },
//     ],
//   };

//   return (
// <div className="px-6 text-center">
// <Slider {...sliderSettings}>
// {details?.map(discount => (
//                         <div key={discount.id} className="p-4">
//                             <div className="max-w-sm mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
//                                 <div className="relative h-[300px]">
//                                     <img
//                                         className="w-full h-full object-cover"
//                                         src={discount.image}
//                                         alt="Product Image"
//                                     />
//                                     <div className="absolute top-0 right-0 bg-red-500 text-white p-2 font-semibold rounded-tr-lg rounded-bl-lg">
//                                         Go for discount
//                                     </div>
//                                 </div>
//                                 <div className="px-6 py-4">
//                                     <h3 className="text-xl font-semibold mb-2">{discount.name}</h3>
//                                     <span className="text-gray-700 text-base">Original Price: ${discount?.price}</span>
//                                 </div>
//                             </div>
//                         </div>
//                     ))}
//     </Slider>
// </div>
//   );
// };

// export default AdvertisementSlider;

// // import Slider from "react-slick";
// // import "slick-carousel/slick/slick.css";
// // import "slick-carousel/slick/slick-theme.css";

// // const Carousel = ({ details }) => {
// //     console.log(details);

// //     const settings = {
// //         dots: true,
// //         infinite: true,
// //         speed: 1000,
// //         autoplay: true,
// //         autoplaySpeed: 1000,
// //         slidesToShow: 3,
// //         slidesToScroll: 0.5,
// //         responsive: [
// //             {
// //                 breakpoint: 768,
// //                 settings: {
// //                     slidesToShow: 1,
// //                     slidesToScroll: 1,
// //                 },
// //             },
// //             {
// //                 breakpoint: 1024,
// //                 settings: {
// //                     slidesToShow: 2,
// //                     slidesToScroll: 1,
// //                 },
// //             },
// //             {
// //                 breakpoint: 1280,
// //                 settings: {
// //                     slidesToShow: 3,
// //                     slidesToScroll: 1,
// //                 },
// //             },
// //         ],
// //     };

// //     return (
// //         <div className="bg-gray-100 mx-4">
// //             <h2 className="text-6xl font-bold text-center border-b-2 pb-4">Discounts !!!</h2>
// //             <div className="mx-auto max-w-screen-lg">
// //                 <Slider {...settings}>
// //                     {details?.map(discount => (
// //                         <div key={discount.id} className="p-4">
// //                             <div className="max-w-sm mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
// //                                 <div className="relative h-[300px]">
// //                                     <img
// //                                         className="w-full h-full object-cover"
// //                                         src={discount.image}
// //                                         alt="Product Image"
// //                                     />
// //                                     <div className="absolute top-0 right-0 bg-red-500 text-white p-2 font-semibold rounded-tr-lg rounded-bl-lg">
// //                                         Go for discount
// //                                     </div>
// //                                 </div>
// //                                 <div className="px-6 py-4">
// //                                     <h3 className="text-xl font-semibold mb-2">{discount.name}</h3>
// //                                     <span className="text-gray-700 text-base">Original Price: ${discount?.price}</span>
// //                                 </div>
// //                             </div>
// //                         </div>
// //                     ))}
// //                 </Slider>
// //             </div>
// //         </div>
// //     );
// // };

// // // export default Carousel;
// // import React, { useState } from "react";
// // import Slider from "react-slick";
// // import "slick-carousel/slick/slick.css";
// // import "slick-carousel/slick/slick-theme.css";

// // const Carousel = ({ details }) => {
// //   const [slidesToShow, setSlidesToShow] = useState(3);

// //   const settings = {
// //     dots: true,
// //     infinite: true,
// //     speed: 1000,
// //     autoplay: true,
// //     autoplaySpeed: 1000,
// //     slidesToShow: slidesToShow,
// //     slidesToScroll: 1,
// //     responsive: [
// //       {
// //         breakpoint: 768,
// //         settings: {
// //           slidesToShow: 1,
// //           slidesToScroll: 1,
// //         },
// //       },
// //       {
// //         breakpoint: 1024,
// //         settings: {
// //           slidesToShow: 2,
// //           slidesToScroll: 1,
// //         },
// //       },
// //       {
// //         breakpoint: 1280,
// //         settings: {
// //           slidesToShow: 3,
// //           slidesToScroll: 1,
// //         },
// //       },
// //     ],
// //   };

// //   const handleSliderChange = (event) => {
// //     setSlidesToShow(Number(event.target.value));
// //   };

// //   return (
// //     <div className="bg-gray-100 mx-4 py-8">
// //       <h2 className="text-4xl font-bold text-center mb-8">Discounts !!!</h2>
// //       <div className="mx-auto max-w-screen-lg">
// //         <div className="mb-4 text-center">
// //           <label htmlFor="slides" className="mr-2">
// //             Number of Slides:
// //           </label>
// //           <input
// //             type="range"
// //             id="slides"
// //             name="slides"
// //             min="1"
// //             max="5"
// //             value={slidesToShow}
// //             onChange={handleSliderChange}
// //             className="w-48"
// //           />
// //           <span className="ml-2">{slidesToShow}</span>
// //         </div>
// //         <div className="slick-container">
// //           <Slider {...settings}>
// //             {details?.map((discount) => (
// //               <div key={discount.id} className="p-4 slick-slide">
// //                 {/* ... your existing slide content ... */}
// //               </div>
// //             ))}
// //           </Slider>
// //         </div>
// //       </div>
// //     </div>
// //   );
// // };

// // export default Carousel;

// import Slider from "react-slick";
// import "slick-carousel/slick/slick.css";
// import "slick-carousel/slick/slick-theme.css";

// const dvertisementSlider = ({ details }) => { 
//     console.log(details);
//     const settings = {
//         dots: true,
//         infinite: true,
//         speed: 1000,
//         autoplay: true,
//         autoplaySpeed: 1000,
//         slidesToShow: 3,
//         slidesToScroll: 1,  // Change this to 1
//         responsive: [
//             {
//                 breakpoint: 768,
//                 settings: {
//                     slidesToShow: 3,
//                     slidesToScroll: 1,
//                 },
//             },
//             {
//                 breakpoint: 1024,
//                 settings: {
//                     slidesToShow: 3,
//                     slidesToScroll: 1,
//                 },
//             },
//             {
//                 breakpoint: 1280,
//                 settings: {
//                     slidesToShow: 3,
//                     slidesToScroll: 1,
//                 },
//             },
//         ],
//     };
    

//     return (
//         <div className="bg-gray-100 mx-4">
//             <h2 className="text-6xl font-bold text-center border-b-2 pb-4">Discounts !!!</h2>
//             <div className="mx-auto">
//                 <Slider {...settings}> 
//                     {details?.map(discount => (
//                         <div key={discount.id} className="p-4">
//                             <div className="max-w-sm mx-auto bg-white shadow-lg rounded-lg ">
//                                 <div className="relative h-[300px]">
//                                     <img
//                                         className="w-full h-full object-cover"
//                                         src={discount.image}
//                                         alt="Product Image"
//                                     />
//                                     <div className="absolute top-0 right-0 bg-red-500 text-white p-2 font-semibold rounded-tr-lg rounded-bl-lg">
//                                         Go for discount
//                                     </div>
//                                 </div>
//                                 <div className="px-6 py-4">
//                                     <h3 className="text-xl font-semibold mb-2">{discount.name}</h3>
//                                     <span className="text-gray-700 text-base">Original Price: ${discount?.price}</span>
//                                 </div>
//                             </div>
//                         </div>
//                     ))}
//                 </Slider>
//             </div>
//         </div>
//     );
// };

// export default dvertisementSlider;


import  { useEffect, useState } from "react";
import Slider from "react-slick";
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";

const AdvertisementSlider = () => {
  const [discounts, setDiscounts] = useState([]);

  useEffect(() => {
    fetch("/advertisement.json")
      .then(res => res.json())
      .then(data => setDiscounts(data))
  }, []);

  const settings = {
    dots: true,
    infinite: true,
    speed: 1000,
    autoplay: true,
    autoplaySpeed: 1000,
    slidesToShow: 3,
    slidesToScroll: 0.3,
    responsive: [
      {
        breakpoint: 425,  
        settings: {
          slidesToShow: 1,
          slidesToScroll: 1,
        },
      },
      {
        breakpoint: 768,
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
        },
      },
     
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
        },
      },
      {
        breakpoint: 1280,
        settings: {
          slidesToShow: 3,
          slidesToScroll: 1,
        },
      },
    ],
  };

  return (
    <div className="bg-gray-100 p-4">
      <h2 className=" text-3xl font-bold text-center border-b-2 pb-4">Our up coming products !!!</h2>
      <div className="mx-auto max-w-screen-lg">
        <Slider className="mx-auto" {...settings}>
          {discounts.map(discount => (
            <div key={discount.id} className="p-4">
              <div className="max-w-sm mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
                <div className="relative h-[300px]"> 
                  <img
                    className="w-full h-full object-cover"
                    src={discount.img}
                    alt="Product Image"
                  />
                  <div className="absolute top-0 right-0 bg-red-500 text-white p-2 font-semibold rounded-tr-lg rounded-bl-lg">
                    $ {discount.originalPrice}
                  </div>
                </div>
                <div className="px-6 py-4">
                  <h3 className="text-xl text-center font-semibold mb-2">{discount.productName}</h3>
                </div>
              </div>
            </div>
          ))}
        </Slider>
      </div>
    </div>
  );
};

export default AdvertisementSlider;